        (function () {
            ...
            const SecurityAndUtils = (function () {
                ...
                function constantTimeCompare(a, b) {
                    ...
                    // Si las longitudes son diferentes, aún hacemos la comparación completa
                    // para evitar revelar información sobre la longitud
            ...
            const DataManagement = (function (S) {
                    ...
                    try {
                        ...
                        if (data.hash) {
                            ...
                            if (!S.constantTimeCompare(hashCalculado, data.hash)) {
                                ...
                                if (data.hash.length < 64 && hashCalculado.length === 64) {
                                    console.log('🔄 Migrando formato de seguridad (FNV1a -> SHA-256)...');

                                    setTimeout(async () => {
                                        await guardarConIntegridad((data.registros || []).filter(S.validarRegistroSeguro));
                                        console.log('✅ Migración de hash completada en segundo plano.');
                                    }, 2000);
                ...
                // >>>>> ESTA FUNCION DE HASH DEBE VOLVER DESPUES DE 2 SEMANAS <<<<<
                ...
                async function guardarYActualizar(actualizarRegistros = true) {
                ...
                async function agregarRegistro() {
                    ...
                    // Si no hay entrada ni salida, decidir qué hacer según el contexto
                    if (!e && !s) {
                        const now = new Date();
                        ...
                        // Si existe registro con entrada pero sin salida, agregar salida
                        if (registroExistente && registroExistente.entrada && !registroExistente.salida) {
                            s = horaActual;
                            $('salida').value = s;
                            usaHoraActual = true;
                        } else {
                            // Si no, crear entrada con hora actual
                            e = horaActual;
                            $('entrada').value = e;
                            usaHoraActual = true;
                        }
                    ...
                    // Detectar si solo quiere agregar salida
                    if (!e && s) {
                    ...
                    // AGREGAR SALIDA DIRECTAMENTE si existe entrada sin salida
                    ...
                    // Validación para registros con ambos campos o solo entrada
                    if (registroExistente) {
                        UILogic.resetearBoton(btn);
                        if (usaHoraActual) {
                            $('entrada').value = '';
            ...
            const UILogic = (function (S, D) {
                ...
                function crearItemRegistroIndividual(r, horasDiarias) {
                ...
                function actualizarListaRegistros(registros) {
                    ...
                    // Guardar estado de grupos expandidos ANTES de limpiar
                    ...
                    // CAMBIO CLAVE: Aplanar todos los items (grupos + individuales)
                    let todosLosItems = [];
                    ...
                    // Filtrar solo items (sin títulos) para contar correctamente
                    ...
                    itemsAMostrar.forEach(item => {
                        // Obtener el mes de este item
                        const fechaItem = item.grupo.registros[0].fecha;
                        const mesItem = fechaItem.substring(0, 7);
                        mesesVisibles.add(mesItem);
                    });

                    // Renderizar solo los items visibles con sus títulos
                    let mesActual = null;
                    ...
                    itemsAMostrar.forEach(itemData => {
                        ...
                        // Agregar título de mes si cambió
                        if (mesItem !== mesActual) {
                            const tituloGrupo = document.createElement('h3');
                            tituloGrupo.className = 'registro-grupo-titulo';
                            tituloGrupo.textContent = formatoTituloMes(mesItem);
                            fragmento.appendChild(tituloGrupo);
                            mesActual = mesItem;
                        }

                        // Agregar el item (grupo o individual)
                        if (grupo.tipo === 'grupo') {
                ...
                // Función auxiliar para crear grupo expandible (agrégala si no existe)
                ...
                function actualizarUI() {
                    ...
                    if (vistaActual === 'semana') {
                        ...
                        const tot = registros.filter(r =>
                            ...
                            !(r.entrada === '11:11' && r.salida === '11:11') && // Ignorar Ausencias
                            !(r.entrada === '22:22' && r.salida === '22:22')    // Ignorar Vacaciones
                        ...
                        if (mensajeEl) {
                            ...
                            if (objetivoAjustado === 0) {
                                ...
                                // Mensaje activo (aún en jornada laboral)
                                mensajeEl.textContent = tot >= objetivoAjustado ?
                        ...
                        if (regHoy && regHoy.entrada) {
                            ...
                            if (isFeriadoHoy) {
                                ...
                                if (progressEl) {
                                    ...
                                    setProgressBarColor(progressEl, 'purple'); // color de barra por feriado
                                }
                                ...
                                if (progressEl) {
                                    progressEl.style.width = '100%';
                                    setProgressBarColor(progressEl, 'purple'); // color de barra por vacaciones
                                }
                                if (mensajeEl) mensajeEl.textContent = `¡Día de vacaciones!`;

                            } else {
                                // Lógica normal de trabajo (horas reales)
                                if (regHoy.salida) {
                                ...
                                if (regHoy.salida) {
                                    ...
                                    if (mensajeEl) mensajeEl.textContent = tiempoTranscurrido >= objetivoDiario ? `Te podes ir! (+${horasATexto(tiempoTranscurrido - objetivoDiario)})` : `Faltan ${horasATexto(faltante)}`;
                                }
                            }
                        } else {
                            if (progressEl) {
...
Generado por Sistema de Horarios version v6.10
`
                    };
                ...
                async function init() {
                    ...
                    const registrosCargados = await D.cargarConVerificacion();
                    ...
                    try {
                        const formExpandido = localStorage.getItem('formularioExpandido') === 'true';
                        if (formExpandido) {
                            toggleFormulario();
                        }
                    } catch (e) { }
                    ...
                    try {
                        const statsExpandido = localStorage.getItem('statsExpandido') === 'true';
                        if (statsExpandido) { toggleStats(); }
                    } catch (e) { }

                    try {
                        const botonesExpandido = localStorage.getItem('botonesHistoricoExpandido') === 'true';
                        if (botonesExpandido) {
                            toggleBotonesHistorico();
                        }
                    } catch (e) { }
                    ...
                    window.UILogic = {
                        ...
                        cerrarFiltros, mostrarAyuda, cerrarAyuda, cambiarMesStats, generarReporte, mostrarVacaciones, cerrarVacaciones, toggleFormulario,
                        toggleBotonesHistorico, toggleStats
                    };
                    ...
                    window.PWAInstaller = {
                        instalarApp: PWAInstaller.instalarApp
                    };
                ...
                function toggleFormulario() {
                    ...
                    const toggleBtn = $('btn-toggle-form');
                    ...
                    if (form.classList.contains('expanded')) {
                        icon.setAttribute('href', '#icon-cancelar');
                    } else {
                    ...
                    try {
                        localStorage.setItem('formularioExpandido', form.classList.contains('expanded'));
                    } catch (e) { }
                }

                function toggleBotonesHistorico() {
                    const botones = $('botones-historico');
                    const toggleBtn = $('btn-toggle-historico');
                    const icon = toggleBtn.querySelector('use');

                    botones.classList.toggle('expanded');
                    toggleBtn.classList.toggle('rotated');

                    // Cambiar icono entre + y -
                    if (botones.classList.contains('expanded')) {
                        icon.setAttribute('href', '#icon-cancelar');
                    } else {
                        icon.setAttribute('href', '#icon-plus');
                    }
                    ...
                    // Guardar estado en localStorage
                    try {
                        localStorage.setItem('botonesHistoricoExpandido', botones.classList.contains('expanded'));
                    } catch (e) { }
                }

                function toggleStats() {
                    const form = $('form-stats');
                    const toggleBtn = $('btn-toggle-stats');
                    const icon = toggleBtn.querySelector('use');
                    ...
                    if (form.classList.contains('expanded')) {
                        icon.setAttribute('href', '#icon-cancelar');
                        // Aseguramos que los datos estén actualizados al abrir
                        poblarSelectorMeses();
                        const selectMes = $('select-mes-stats');
                    ...
                    // Guardar estado en localStorage
                    try {
                        localStorage.setItem('statsExpandido', form.classList.contains('expanded'));
                    } catch (e) { }
                }
                ...
                return {
                    ...
                    cerrarAyuda, cambiarMesStats, generarReporte, toggleBotonesHistorico, toggleStats
                };
            ...
            window.addEventListener('load', async function () {
                try {
                    ...
                    if (hashGuardado && hashGuardado !== hash) {
                        ...
                        if (window.UILogic) {
                            UILogic.mostrarToast('SW Actualizado', 'info');
                        }
                    }
